...
type ConfigTable struct {
	...
	TsInfoByID map[int32]*TsInfoDefine
}
...
func NewConfigTable() *ConfigTable {
	return &ConfigTable{
		...
		indexFuncByName: map[string][]func(*ConfigTable) error{
			...
			"TsInfo": {func(tab *ConfigTable) error {

				// TsInfo
				for _, def := range tab.TsInfo {

					if _, ok := tab.TsInfoByID[def.ID]; ok {
						panic(fmt.Sprintf("duplicate index in TsInfoByID: %v", def.ID))
					}

					tab.TsInfoByID[def.ID] = def
		...
		clearFuncByName: map[string][]func(*ConfigTable) error{
			...
			"TsInfo": {func(tab *ConfigTable) error {
				...
				tab.TsInfoByID = make(map[int32]*TsInfoDefine)

				return nil
			}},
		},

		TsInfoByID: make(map[int32]*TsInfoDefine),
	}
}
