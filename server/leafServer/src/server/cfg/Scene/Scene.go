...
type ConfigTable struct {
	...
	SceneByName map[string]*SceneDefine
}
...
func NewConfigTable() *ConfigTable {
	return &ConfigTable{
		...
		indexFuncByName: map[string][]func(*ConfigTable) error{
			...
			"Scene": {func(tab *ConfigTable) error {

				// Scene
				for _, def := range tab.Scene {

					if _, ok := tab.SceneByName[def.Name]; ok {
						panic(fmt.Sprintf("duplicate index in SceneByName: %v", def.Name))
					}

					tab.SceneByName[def.Name] = def
		...
		clearFuncByName: map[string][]func(*ConfigTable) error{
			...
			"Scene": {func(tab *ConfigTable) error {
				...
				tab.SceneByName = make(map[string]*SceneDefine)

				return nil
			}},
		},

		SceneByName: make(map[string]*SceneDefine),
	}
}
