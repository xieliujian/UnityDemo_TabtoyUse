...
func NewConfigTable() *ConfigTable {
	return &ConfigTable{
		...
		indexFuncByName: map[string][]func(*ConfigTable) error{
			...
			"Hero": {func(tab *ConfigTable) error {

				// Hero
				for _, def := range tab.Hero {

					if _, ok := tab.HeroByID[def.ID]; ok {
						panic(fmt.Sprintf("duplicate index in HeroByID: %v", def.ID))
					}

					tab.HeroByID[def.ID] = def
		...
		clearFuncByName: map[string][]func(*ConfigTable) error{
			...
			"Hero": {func(tab *ConfigTable) error {
				...
				tab.HeroByID = make(map[int64]*HeroDefine)

				return nil
			}},
		},

		HeroByID: make(map[int64]*HeroDefine),
	}
}
